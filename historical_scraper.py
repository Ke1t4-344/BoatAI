...
def process_venue(conn: sqlite3.Connection, date_str: str, vcode: str) -> bool:
    _upsert_venue(conn, vcode)
    max_rno = fetch_max_race_no(vcode, date_str)
    if _stop:
        return False
    ...
    for rno in range(1, max_rno + 1):
        ...
        race_id = _upsert_race(conn, date_str, vcode, rno)
        ...
        if soup:
            ...
            _save_entries(conn, race_id, entries)
        ...
        if soup:
            ...
            _save_race_result_entries(conn, race_id, res_entries)
            _save_payouts(conn, race_id, payouts)
