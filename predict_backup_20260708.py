...
WEIGHTS = {
    "course_win_rate":     0.5059,
    "national_win_rate":   0.2510,
    "local_win_rate":      0.0515,
    "motor_2ring":         0.0558,
    "exhibition_time":     0.1358,
    "start_timing":        0.0001,
}
...
def predict(date: str, venue_code: str, race_no: int) -> dict:
    ...
    if not honmei_top5:
        honmei_top5 = by_prob[:5]
    if not chuana_top5:
        chuana_top5 = by_prob[5:10]
    if not ana_top5:
        ana_top5 = sorted(by_prob[10:30], key=lambda x: x["ev"] or 0, reverse=True)[:5]
